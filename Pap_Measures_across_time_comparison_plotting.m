...
int_ict=1;
X_all_norm_soz_int_in=squeeze(nanmean(nanmean(nanmean(X_all_normALL(:,targ_feat,1,:,trials,epoch,int_ict),6),5),1));
X_all_norm_soz_int_in=X_all_norm_soz_int_in(ppr_ordrds,:);
X_all_norm_non_int_in=squeeze(nanmean(nanmean(nanmean(X_all_normALL(:,targ_feat,2,:,trials,epoch,int_ict),6),5),1));
X_all_norm_non_int_in=X_all_norm_non_int_in(ppr_ordrds,:);

int_ict=2;
X_all_norm_soz_ict_in=squeeze(nanmean(nanmean(nanmean(X_all_normALL(:,targ_feat,1,:,trials,epoch,int_ict),6),5),1));
X_all_norm_soz_ict_in=X_all_norm_soz_ict_in(ppr_ordrds,:);
X_all_norm_non_ict_in=squeeze(nanmean(nanmean(nanmean(X_all_normALL(:,targ_feat,2,:,trials,epoch,int_ict),6),5),1));
...
int_ict=1;
X_all_norm_soz_int_out=squeeze(nanmean(nanmean(nanmean(X_all_normALL(:,targ_feat,1,:,trials,epoch,int_ict),6),5),1));
X_all_norm_soz_int_out=X_all_norm_soz_int_out(ppr_ordrds,:);
X_all_norm_non_int_out=squeeze(nanmean(nanmean(nanmean(X_all_normALL(:,targ_feat,2,:,trials,epoch,int_ict),6),5),1));
X_all_norm_non_int_out=X_all_norm_non_int_out(ppr_ordrds,:);

int_ict=2;
X_all_norm_soz_ict_out=squeeze(nanmean(nanmean(nanmean(X_all_normALL(:,targ_feat,1,:,trials,epoch,int_ict),6),5),1));
X_all_norm_soz_ict_out=X_all_norm_soz_ict_out(ppr_ordrds,:);
X_all_norm_non_ict_out=squeeze(nanmean(nanmean(nanmean(X_all_normALL(:,targ_feat,2,:,trials,epoch,int_ict),6),5),1));
...
for feat=1:13
    c=c+4;
    if strcmp(ictal_or_inter,'interictal')
        data1=X_all_norm_soz_int_in;
        data2=X_all_norm_soz_int_out;
    else
        data1=X_all_norm_soz_ict_in;
        data2=X_all_norm_soz_ict_out;
    end
    % if strcmp(ictal_or_inter,'interictal')
    %     data1=X_all_norm_non_int_in;
    %     data2=X_all_norm_non_int_out;
    % else
    %     data1=X_all_norm_non_ict_in;
    %     data2=X_all_norm_non_ict_out;
    % end
    ...
    colours=[0.9 0.3 0.3];
    % colours=[0.3 0.3 0.9];
    data_tmp2=data2(feat,~isnan(data2(feat,:)));
...
boxplot(datas_tmp_1_outed,cs1,'Positions',cs1,'Whisker',inf,'color',[0.5 0.3 0.3]);
% boxplot(datas_tmp_1_outed,cs1,'Positions',cs1,'Whisker',inf,'color',[0.3 0.3 0.5]);
hold on;
boxplot(datas_tmp_2_outed,cs2,'Positions',cs2,'Whisker',inf,'color',[0.9 0.3 0.3]);
% boxplot(datas_tmp_2_outed,cs2,'Positions',cs2,'Whisker',inf,'color',[0.3 0.3 0.9]);
box off
