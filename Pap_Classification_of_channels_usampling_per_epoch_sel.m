...
for Patient=all_patients % patient indices
    ...
    for trial=trls
        for epoch=1:3
                ...
                for iter_rand=1:1
                    ...
                    for fld = 1:CVO.NumTestSets
                        ...
                        if length(unique(Y_train))>1
                            ...
                            % balancing classse through upsampling
                            ...
                            impCART{p,trial,epoch,iter_equalis,iter_rand,fld} = Classifier_Model.OOBPermutedPredictorDeltaError;
                            preds_tmp=predict(Classifier_Model,normalize(X_test));

                            for i=1:length(preds_tmp)
                                preds(i,1)=str2num(preds_tmp{i});
                            end
                            Predictions{p,trial,epoch,iter_equalis,iter_rand,fld}=preds;
                            classif=nanmean(preds==Y_test);
                            Ground_truth{p,trial,epoch,iter_equalis,iter_rand,fld}=Y_test;
                            clearvars preds
                        else
                            Predictions{p,trial,epoch,iter_equalis,iter_rand,fld}=nan;
                            Ground_truth{p,trial,epoch,iter_equalis,iter_rand,fld}=nan;
                            impCART{p,trial,epoch,iter_equalis,iter_rand,fld} = nan;
                        end
                    end
                end
                clearvars X y y_r
            ...
            if strcmp(ictal_or_inter,'ictal')
                save(['Connectiv_classif_oversamp_Seizure.mat'],'Predictions','Ground_truth','impCART','-v7.3');
            elseif strcmp(ictal_or_inter,'interictal')
                save(['Connectiv_classif_oversamp_Interictal.mat'],'Predictions','Ground_truth','impCART','-v7.3');
            end
            [Patient trial epoch]
        end
    end
...
t=0;
if strcmp(ictal_or_inter,'ictal')
    load(['Connectiv_classif_oversamp_Seizure.mat']);
    trls=5;
elseif strcmp(ictal_or_inter,'interictal')
    load(['Connectiv_classif_oversamp_Interictal.mat']);
    trls=2;
end
    ...
    for epoch=1:3
        ...
        for p=1:size(Ground_truth,1) % patients
            for fld=1:size(Ground_truth,6) % folds
                for iter_equalis=1:size(Ground_truth,4) % iterations
                        Performance_within(p,trial,epoch,iter_equalis,iter_rand,fld,1:7)=Evaluate(Ground_truth{p,trial,epoch,iter_equalis,iter_rand,fld},Predictions{p,trial,epoch,iter_equalis,iter_rand,fld});
                    try
                        [~,~,~,Performance_within(p,trial,epoch,iter_equalis,iter_rand,fld,8)]=perfcurve(Ground_truth{p,trial,epoch,iter_equalis,iter_rand,fld},Predictions{p,trial,epoch,iter_equalis,iter_rand,fld},1);
                    catch
                        Performance_within(p,trial,epoch,iter_equalis,iter_rand,fld,8)=nan;
                    end
                end
            end
        end
        ...
        Performance_within=squeeze(nanmean(nanmean(Performance_within(:,trial,epoch,:,:,:,:),6),4));
        % Performance_rand_within=squeeze(nanmean(nanmean(Performance_rand_within,4),2));
        ...
        title(['Median = ',sprintf('%0.2f',nanmedian(datas_tmp_outed))])
        box off
        grid on
        plot([0.5 2.5],[0.5 0.5],'--k')
        ylim([0.3 1])
        clearvars datas datas_tmp_outed
        % bays_against_chance=[bf.ttest2(datas(:,1),datas_random(:,1))]
        % bays_diff=bf.ttest(datas(:,1))
        % [mean(datas(:,1)) std(datas(:,1))]
    end
end
ccc
...
%     'igci'};
% 
% % net_feats={'-InS','-OutS','-SrTim','-DsTim','-ClsCof','-Eccnt','-NodBtw','-EdgBtw'};
...
% end
% 
% feats=1:size(impCART{1,1,1,1,1,1},2);
% 
% ff=1;
% iter_rand=1;
% iter_equalis=1;
% feat_imp=nan(size(impCART,1),size(Features_labels,2),trls,3,size(impCART,6));
% for p=1:size(impCART,1)
%     for trial=1:trls
%         for epoch=1:size(impCART,3)
%             for fld=1:size(impCART,6)
%                 if size(impCART{p,trial,epoch,iter_equalis,iter_rand,fld},1)~=0
%                     feat_imp(p,:,trial,epoch,fld)=impCART{p,trial,epoch,iter_equalis,iter_rand,fld};
%                 else
%                     feat_imp(p,:,trial,epoch,fld)=nan;
%                 end
%             end
%         end
%     end
...
% Collapsing across connectivities
datas=squeeze(nanmean(nanmean(nanmean(feat_imp,5),4),3));
subplot(121)
...
for cat=1:length(connectivities)
    ...
    % datas_new(:,cat)=nanmean(datas(:,inds_inc),2);
    datas_new(:,cat)=reshape(datas(:,inds_inc),[],1);    
end
